...
impl Config {
    ...
    pub fn activitywatch_url(&self) -> String {
        format!(
            "http://{}:{}",
            self.activitywatch.host, self.activitywatch.port
        )
    }
}
    ...
    fn default() -> Self {
        ...
        Self {
            ...
            storage: StorageConfig {
                s3: S3Config {
                    enable: false,
                    bucket: "aw-screenshots".to_string(),
...
mod tests {
    ...
    fn test_load_config() {
        ...
        assert_eq!(config.activitywatch.host, "localhost");
        assert_eq!(config.activitywatch.port, 5600);
    }
